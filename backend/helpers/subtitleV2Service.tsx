import { subtitleOpenAI, SubtitleProviderError } from './subtitleOpenAI';
...
async function execute(request:Request,kind:'original'|'translation',input:any) {
 ...
 try {
   ...
   let result:unknown;
   if(kind==='original') {
     const raw=await subtitleOpenAI.original(input.src);
   ...
   if(error instanceof SubtitleProviderError)return json({code:error.code,error:error.status===429?'Слишком много запросов. Попробуйте через минуту.':'Распознавание временно недоступно. Попробуйте позже.'},error.status);
